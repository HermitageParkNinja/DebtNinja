...
// ── Create payment plan ──
export async function createPaymentPlan(debtorId, monthlyAmount, numMonths) {
  const res = await fetch('/api/payment-plan', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ debtor_id: debtorId, monthly_amount: monthlyAmount, num_months: numMonths }),
  })
  return res.json()
}
